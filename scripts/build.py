...
def image(name, alt, prefix, cls='', eager=False, position=None):
    ...
    return f'<img src="{prefix}assets/images/{E(name)}" alt="{E(alt)}" class="{cls}"{style} loading="{"eager" if eager else "lazy"}" decoding="async">'
...
def projectpage(lang,p):
    ...
    for related_id in p.get('related_projects',[]):
        related=next(x for x in DATA['projects'] if x['id']==related_id)
        label='Related work' if lang=='en' else 'Obra relacionada'
        links+=f'<a href="../{related_id}/">{label}: {E(related[lang]["title"])} ↗</a>'
    hero=image(p['image'],q['alt'],prefix,'project-hero',True)
    if p.get('hero_pair'):
        hero=f'<div class="project-hero-pair">{image(p["image"],q["alt"],prefix,eager=True)}{image(p["hero_pair"],q["hero_pair_alt"],prefix,eager=True)}</div>'
    if p.get('preview_link'):
        preview=next(x for x in p['links'] if x['url']==p['preview_link'])
        hero=f'<a class="project-video" href="{E(preview["url"])}" aria-label="{E(preview[lang])}">{hero}<span class="preview-label">{E(preview[lang])} ↗</span></a>'
    ...
    detail=''
    if p['id']!='biocenosis':
        photo=image(p['detail_image'],q['detail_alt'],prefix,'detail-photo') if p.get('detail_image') else ''
        if p.get('detail_layout')=='portrait':
            detail=f'<section class="project-detail portrait-detail"><figure>{photo}<figcaption class="caption">{E(q["image_credit"])}</figcaption></figure><div><h2>{E(q["detail_title"])}</h2>{paras(q["detail"])}</div></section>'
        else:
            detail=f'<section class="project-detail"><div class="detail-text"><h2>{E(q["detail_title"])}</h2><div>{paras(q["detail"])}</div></div>{photo}{gallery}</section>'
    nxt=DATA['projects'][(int(p['number']))%len(DATA['projects'])]
    home=prefix+('es/' if lang=='es' else '')
    body=f'''<main class="wrap" id="main"><section class="project-intro"><p class="eyebrow">{p['number']} / {p['year']} / {E(q['medium'])}</p><h1 class="project-title">{E(q['title'])}</h1><p class="project-subtitle">{E(q['subtitle'])}</p><p class="project-credit">{E(p['credit'])}</p></section><figure>{hero}<figcaption class="caption"><span>{E(q['title'])} · {p['year']}</span><span>{E(q.get('image_credit',p['credit']))}</span></figcaption></figure><section class="project-text"><div class="project-body"><p class="lead">{E(q['summary'])}</p>{paras(q['body'])}</div><aside><dl class="project-facts">{facts}</dl><div class="project-links">{links}</div></aside></section>{detail}<nav class="other-works" aria-label="{'Other works' if lang=='en' else 'Otras obras'}"><a href="{home}#works"><small>{E(t['work_heading'])}</small><strong>← {'All works' if lang=='en' else 'Todas las obras'}</strong></a><a href="../{nxt['id']}/"><small>{'Next work' if lang=='en' else 'Siguiente obra'}</small><strong>{E(nxt[lang]['title'])} →</strong></a></nav></main>'''
